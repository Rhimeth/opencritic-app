...
from ml import GameRecommender, ScorePredictor
...
def main() -> None:
    ...
    if args.refresh:
        refresh_stats = client.refresh(pages=args.pages, page_delay_seconds=args.refresh_delay)
        results["refresh"] = refresh_stats
        results["steps"].append("refresh")

    if args.news:
        news_service = GamingNewsService(enable_deduplication=True)   # enable dedup
    ...
    if frame.empty and needs_games:
        print("No data in database yet. Run with --refresh first (requires OPENCRITIC_API_KEY).")
