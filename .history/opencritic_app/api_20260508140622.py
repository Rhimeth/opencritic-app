...
def recommend_games(game: RecommendRequest):
    recs = recommender.recommend(req.title, request.k)
...
def get_analytics_stats():
    ...
    def extract_series(title):
        words = title.split()
        if len(words) >= 2 and words.lower() in ['super', 'the', 'call', 'final', 'grand', 'resident', 'street', 'mortal', 'assassin', 'god', 'metal', 'borderlands']:
            return f"{words[0]} {words[1]}" if len(words) > 1 else words[0]
        return words
