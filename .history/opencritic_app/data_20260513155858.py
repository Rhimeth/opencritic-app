...
class OpenCriticDataClient:
    ...
    sort_by: str = "date"                     # NEW: sort order
) -> Dict[str, Any]:
    """Fetch new data and update local database incrementally.

    Args:
        pages: Number of API pages to fetch.
        page_size: Games per page (max 20).
        page_delay_seconds: Delay between pages to avoid rate limiting.
        sort_by: Sort order – "date", "score", or "num-reviews".
    """
    if not self.api_key:
        raise ValueError(
